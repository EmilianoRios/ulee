...
const FORM_ID = 'config-form-feriados'

export default function ConfiguracionFeriadosPage() {
  const t = useTheme()
  ...
  const handleSubmit = useCallback(async (holidays: HolidayEntry[]) => {
    if (!venueId) return
    await updateHolidays({ venueId, holidays })
  }, [venueId, updateHolidays])
